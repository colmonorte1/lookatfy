...
export async function GET(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data: booking, error } = await supabase
      .from('bookings')
      .select('id, start_at, service:services!service_id(duration), expert:experts!expert_id(profile:profiles(full_name))')
      .eq('id', id)
      .single()
    ...
    const ics = buildICS({
      ...
      description: `Reserva generada en Lookatfy`,
    ...
    return new NextResponse(ics, {
      ...
      headers: {
        ...
        'Content-Disposition': `attachment; filename="lookatfy-${booking.id}.ics"`,
      },
    })
  } catch (e) {
