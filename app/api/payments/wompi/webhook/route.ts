  ...
  try {
    ...
    if (status === "APPROVED") {
      try {
        ...
        try {
          await fetch(
            `${process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000"}/api/bookings/${reference}/email`,
            {
              method: "POST",
            },
          );
        ...
        try {
          await fetch(
            `${process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000"}/api/notifications/booking`,
            {
              method: "POST",
